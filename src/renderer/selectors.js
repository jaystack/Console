...
export const getAccount = _id =>
  createSelector([ getAccounts ], accounts => accounts.find(account => account._id === _id));

export const getConversations = accountId =>
  createSelector([ getAccount(accountId) ], account => account.conversations);

export const getUsers = accountId => createSelector([ getAccount(accountId) ], account => account.users);
  ...
  createSelector([ getConversations(accountId), getUsers(accountId) ], (conversations, users) =>
    conversations.map(conversation => {
      switch (conversation.type) {
        ...
        case 'group':
          return { ...conversation, users: conversation.userIds.map(userId => users.find(user => user.id === userId)) };
        default:
          return conversation;
      }
    })
